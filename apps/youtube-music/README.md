...
| 上下 | ホーム画面でセクションを移動 / 一覧では項目を移動 / **再生画面: ↑歌詞・↓スリープタイマー** |
| ○ | 決定（プレイリストを開く / 再生開始 / ログイン開始） |
| × | 前の画面へ戻る（ログイン中は中止） |
| △ | 再生画面: 一時停止 / ホーム画面: オフライン ライブラリ / 検索画面: 再検索 |
| □ | ダウンロード（ホーム: 選択中の項目 / プレイリスト: 全曲） / **再生画面: この曲からラジオ** / オフライン画面: 削除 |
| L / R | 前の曲 / 次の曲（ホームでは 1 ページ送り） |
| SELECT | ホーム画面: **検索**（OSK で入力） / 再生画面: **再生モード切替** |
| START | 終了 |
